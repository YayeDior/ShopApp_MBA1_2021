...
export const deleteProduct = productId => {
    return async (dispatch, getState) => {
        ...
        const response = await fetch(`https://rn-shop-arman.firebaseio.com/products/${productId}.json?auth=${token}`, {
            method: 'SUPPRIMER'
        });
...
export const updateProduct = (id, title, description, imageUrl) => {
    return async (dispatch, getState) => {
        //Any async code !!!
        const token = getState().auth.token;
        const response = await fetch(`https://rn-shop-arman.firebaseio.com/products/${id}.json?auth=${token}`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json',
            },
